...
export default function ResponseManualRegister({ isOpen, onClose, onSave, userName, isEdit, initialData }: Props): React.ReactElement | null {
  ...
  const hasChanges =
    ...
    newFiles.length > 0 ||
    existingFiles.map(file => file.id).sort((a, b) => a - b).join(",") !== initialSnapshot.fileIds.slice().sort((a, b) => a - b).join(",")
  ...
  const editedFields: Record<string, boolean> = {
    ...
    fileUpload: newFiles.length > 0 || existingFiles.map(file => file.id).sort((a, b) => a - b).join(",") !== initialSnapshot.fileIds.slice().sort((a, b) => a - b).join(","),
  }
    ...
    <div className={DIALOG_STYLES.overlay}>
      <div className={DIALOG_STYLES.containerLg}>
        <div className={DIALOG_STYLES.header}>
          <h2 className={DIALOG_STYLES.title}>대응매뉴얼 {isEdit ? "편집" : "등록"}</h2>
