...
// API response to table row mapping (is_notice=0: 자료실)
const mapResourcePostToRow = (post: NoticeListPost): ResourceRow => {
  const rawDate = post.date || (post.created_at ? post.created_at.slice(0, 10) : "")
  const p = post as NoticeListPost & { user_name?: string }
  return {
    id: post.id,
    title: post.title,
...
export default function ResourcesList() {
  ...
  // mock
  const [mockData, setMockData] = useState<ResourceRow[]>(resourcesMockData as ResourceRow[])
  const {
  ...
  const { currentPage: mockCurrentPage, totalPages: mockTotalPages, currentData: mockCurrentData, onPageChange: mockOnPageChange } = usePagination<ResourceRow>(mockFilteredData, 30)

  // BE API
  const [apiData, setApiData] = useState<ResourceRow[]>([])
  ...
  const fetchResourceList = useCallback(async () => {
    ...
    try {
      const params: NoticeListRequest = { page: apiCurrentPage, is_notice: 0 }
      ...
      if (response.code === 200) {
        setApiData(response.posts.map(mapResourcePostToRow))
  ...
  useEffect(() => {
    if (!USE_MOCK_DATA) fetchResourceList()
  }, [fetchResourceList])
  ...
  // 데이터 분기
  const data = USE_MOCK_DATA ? (mockFilteredData as ResourceRow[]) : apiData
    ...
    <section className="w-full bg-white">
      ...
      <div className="overflow-x-auto bg-white">
        <DataTable
          ...
          onManageClick={row => {
            setSelectedRow(row as ResourceRow)
            setIsEditMode(true)
            setModalOpen(true)
          }}
        />
